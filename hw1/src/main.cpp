...
int main(int argc, char **argv)
{
    ...
    //	glutMainLoop();
    init(string(argv[0]).substr(0, string(argv[0]).find_last_of("/\\")) + "/");
    //    cout << "Active Matrix: " << canvas.active_matrix << endl;
    //    cout << canvas.matrices[canvas.active_matrix] << endl << endl;
    //    vec3f direction(1,1,1);
    //    canvas.translate(direction);
    //    canvas.translate(direction);
    //    cout << canvas.matrices[canvas.active_matrix];
    mat4f Rz = identity<float, 4, 4>();
    float sin_val = sin(degtorad(-90));
    float cos_val = cos(degtorad(90));
    cout << Rz << endl;
    Rz[0][0] = sin_val;
    Rz[0][1] = cos_val;
    cout << Rz << endl;
